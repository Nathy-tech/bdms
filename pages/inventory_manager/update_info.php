...
// Fetch inventory manager's current information
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    $stmt = $mysqli->prepare("SELECT name, email, phone, profile_picture FROM users WHERE id = ?");
    $stmt->bind_param('i', $inventory_manager_id);
    $stmt->execute();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($password)) {
        ...
        if ($profile_picture) {
            $sql = "UPDATE users SET name=?, phone=?, address=?, profile_picture=?, password=? WHERE id=?";
            $stmt = $mysqli->prepare($sql);
            $stmt->bind_param('sssssi', $name, $phone, $address, $profile_picture, $hashed_password, $inventory_manager_id);
        } else {
            $sql = "UPDATE users SET name=?, phone=?, address=?, password=? WHERE id=?";
            $stmt = $mysqli->prepare($sql);
            $stmt->bind_param('ssssi', $name, $phone, $address, $hashed_password, $inventory_manager_id);
        }
    } else {
        if ($profile_picture) {
            $sql = "UPDATE users SET name=?, phone=?, address=?, profile_picture=? WHERE id=?";
            $stmt = $mysqli->prepare($sql);
            $stmt->bind_param('ssssi', $name, $phone, $address, $profile_picture, $inventory_manager_id);
        } else {
            $sql = "UPDATE users SET name=?, phone=?, address=? WHERE id=?";
            $stmt = $mysqli->prepare($sql);
            $stmt->bind_param('sssi', $name, $phone, $address, $inventory_manager_id);
        }
    }

    // Execute the update query
    if ($stmt->execute()) {
        header('Location: index.php'); // Redirect to inventory manager dashboard
        exit();
    } else {
